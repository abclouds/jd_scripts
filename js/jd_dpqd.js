...
/* const token = [
  '8F8DBD110615E78C41D30A9102EA989D',//1天，10豆；3天，50豆；10天，100豆；
  'B5DE18AEF869FCE64DDBAE0E780999E8',//7天，100豆；
  '0E70AA30DBE7748B9DDACE4F3B2606B2',//7天，40豆；15天，100豆；30天，100豆;
...
!(async () => {
  await requireConfig();
  if(!$.token.length > 0){
	 await notify.sendNotify(`${$.name}`, `获取店铺Token失败请检查`);
     return; 
  }else{
	  token = [...($.token || [])];
  }
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    $.token = await getAuthorShareCode('https://raw.githubusercontent.com/FearNoManButGod/AuthorCode/main/jd_shopToken.json')
	if (!$.token) {
		$.http.get({url: 'https://purge.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_shopToken.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
		await $.wait(1000)
