...
活动入口：各处的签到汇总
Node.JS专用
IOS软件用户请使用 https://gitee.com/abclouds/jd_scripts/raw/master/js/jd_bean_sign.js
更新时间：2021-05-16
...
let JD_DailyBonusPath = "./jd_bean_sign_all.js";
let outPutUrl = './';
...
!(async () => {
    ...
    if (!await fs.existsSync(JD_DailyBonusPath)) {
        console.log(`\njd_bean_sign_all.js 文件不存在，停止执行${$.name}\n`);
        await notify.sendNotify($.name, `本次执行${$.name}失败，jd_bean_sign_all.js 文件下载异常，详情请查看日志`)
        return
    }
...
async function downFile() {
    ...
    await downloadUrl();
    if ($.body) {
        url = 'https://gitee.com/abclouds/jd_scripts/raw/master/js/jd_bean_sign_all.js';
    } else {
        url = 'https://gitee.com/abclouds/jd_scripts/raw/master/js/jd_bean_sign_all.js';
    }
    try {
        ...
        console.log(`jd_bean_sign_all.js文件下载完毕\n\n`);
    } catch (e) {
        console.log("jd_bean_sign_all.js 文件下载异常:" + e);
    }
}
...
function downloadUrl(url = 'https://gitee.com/abclouds/jd_scripts/raw/master/js/jd_bean_sign_all.js') {
    return new Promise(resolve => {
        ...
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    // console.log(`${JSON.stringify(err)}`)
                    console.log(`检测到您当前网络环境不能访问外网,将使用jsdelivr CDN下载jd_bean_sign_all.js文件`);
                    await $.http.get({
                        url: `https://gitee.com/abclouds/jd_scripts/raw/master/js/jd_bean_sign_all.js`,
                        timeout: 10000
                    }).then((resp) => {
                        if (resp.statusCode === 200) {
                            console.log(`jd_bean_sign_all.js文件jsdelivr CDN缓存刷新成功`)
                        }
                    });
                } else {
                    $.body = data;
...
function requireConfig() {
    return new Promise(resolve => {
        ...
        //   jd_bean_sign_allPath = err ? '/tmp/jd_bean_sign_all.js' : JD_DailyBonusPath;
        ...
        JD_DailyBonusPath = process.env.TENCENTCLOUD_RUNENV === 'SCF' ? '/tmp/jd_bean_sign_all.js' : JD_DailyBonusPath;
